...
**中文：** 2.0 大版本更新——样式引擎。（1）样式 API 支持"整图重绘型"（replaces_watermark），对现有插件完全向后兼容。（2）新增「模糊卡片」样式：模糊深色背景 + 中央清晰照片（圆角/阴影/描边）+ 底部相机参数信息栏（字号自适应防溢出），文字水印只画在模糊背景。（3）整图重绘样式可与兼容叠加样式共存：模糊卡片 + 相机品牌 logo 同时显示（logo 带大小/位置/旋转/透明度滑块）。（4）图片水印插件新增 10 个相机品牌 logo 预设（佳能/索尼/尼康/富士/哈苏/徕卡/奥林巴斯/松下/宾得/理光，白色+透明版，2048px）。

## What's New / 新增
- Style API: replaces_watermark (whole-image redraw) + backward compatible / 样式 API 整图重绘型，向后兼容
- New "Blur Card" style (blur bg + clear fg + info bar) / 新增「模糊卡片」样式
- Overlay brand-logo image watermarks with Blur Card / 模糊卡片 + 品牌 logo 同屏叠加
- 10 camera-brand logo presets / 图片水印插件新增 10 个相机品牌 logo 预设
